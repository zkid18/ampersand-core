...
Model resolution (first hit wins):
  1. {AMPERSTAND_DATA_DIR}/.classifier/model.joblib  — user-retrained, takes
     precedence so `amperstand-admin classifier retrain` can override the
     bundled default without code redeploys.
  2. src/amperstand_core/models/newsletter_classifier.joblib  — bootstrap
...
def reload_model() -> None:
    ...
    Called by `amperstand-admin classifier retrain` after writing a new model;
