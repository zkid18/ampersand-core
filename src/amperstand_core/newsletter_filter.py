...
semantics. `amperstand-admin classifier domains` prints what's currently in
effect.
"""
...
from amperstand_core.newsletter_classifier import predict_keep
from amperstand_core.newsletter_domains import load_domains
...
def is_newsletter(msg: EmailMessage) -> bool:
    ...
    - Anything else → reject. Sender can be added by hand via
      `amperstand email allow <sender>` if it's something we actually want.
