"""Top-level typer app for `amperstand-admin`.

Run on the box (over SSH) as the service user. Reads config from process env
plus an optional env-file (default `/etc/amperstand/env`). No HTTP surface.
"""
...
from amperstand_core.server.admin_cli.config import DEFAULT_ENV_FILE, resolve_config

app = typer.Typer(
    name="amperstand-admin",
    help="Out-of-band admin for the amperstand vault server. Run via SSH on the box.",
...
EnvFileOpt = Annotated[
    ...
    typer.Option(
        "--env-file",
        help="Path to KEY=VALUE env file. Defaults to /etc/amperstand/env if present.",
        exists=False,
    ),
]
...
def rotate_key(
    ...
    """Mint a new AMPERSTAND_API_KEY and write it to the env file."""
