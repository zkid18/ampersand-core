...
from amperstand_core.server.feed_registry import FeedRegistry
...
def reset_registry_cache() -> None:
    ...
    AMPERSTAND_DATA_DIR. Used by tests."""
    global _registry_singleton
...
def sync_all_feeds(
    ...
    loop on each. Called by `amperstand-feed-sync.timer` on the droplet —
    that systemd unit used to invoke the CLI's `amperstand feed sync` which
