...
Self-hoster's S2/S4 finding: feeds historically lived in `~/.amperstand/state.json`
on whichever client ran `amperstand feed add`. A laptop's `feed add` never
...
`POST /feeds`; `amperstand-feed-sync.timer` pulls the list from the same table.
