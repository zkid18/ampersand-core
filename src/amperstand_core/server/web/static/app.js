/* Amperstand vault web view — vanilla JS, hash-routed.
 *
 * Three views:
 ...
 *
 * Auth: paste AMPERSTAND_API_KEY once, store in localStorage, attach as
 * Bearer to every fetch. 401 → clear + reload.
 */

const KEY_STORAGE = "amperstand_api_key";
const PAGE_SIZE = 20;
...
function getKey() {
  ...
  while (!k) {
    k = window.prompt("Paste your AMPERSTAND_API_KEY (one-time):");
    if (k && k.trim()) {
