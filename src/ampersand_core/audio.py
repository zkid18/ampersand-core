...
1. yt-dlp downloads just the audio track as MP3 (smaller than the full
   ...
   ~30 min). Routes through AMPERSAND_HTTP_PROXY when set so the cloud
   IP block doesn't bite.
2. The MP3 goes to OpenAI's Whisper API which returns plain-text
   transcript. ~$0.006/min — gated behind AMPERSAND_YOUTUBE_AUDIO_FALLBACK
...
# in — the systemd service unit doesn't put /opt/ampersand/venv/bin on
...
# AMPERSAND_YOUTUBE_AUDIO_MAX_SEC. Prevents a 3-hour podcast surprise.
...
def youtube_audio_fallback_enabled() -> bool:
    """Reads AMPERSAND_YOUTUBE_AUDIO_FALLBACK. Off unless explicitly truthy."""
    return os.environ.get("AMPERSAND_YOUTUBE_AUDIO_FALLBACK", "").strip().lower() in {
        "1", "true", "yes", "on",
    }


def youtube_audio_max_seconds() -> int:
    raw = os.environ.get("AMPERSAND_YOUTUBE_AUDIO_MAX_SEC", "").strip()
    try:
...
def transcribe_youtube(
    ...
    if duration is not None and duration > cap:
        raise AudioError(
            ...
            f"set AMPERSAND_YOUTUBE_AUDIO_MAX_SEC to override"
        )
