"""ampersand-admin classifier retrain — fold user feedback into the model.

Inputs:
  bundled  src/ampersand_core/data/labeled.jsonl  (LLM-bootstrap, immutable)
  bundled  src/ampersand_core/data/holdout.jsonl  (frozen eval, never train)
...
The watcher (ampersand-email-watch) caches the loaded model in-process, so
after a swap you should restart it: `systemctl restart ampersand-email-watch`.
"""
...
def run(config: AdminConfig, *, force: bool = False) -> None:
    ...
    print(
        ...
        "  systemctl restart ampersand-email-watch"
    )
