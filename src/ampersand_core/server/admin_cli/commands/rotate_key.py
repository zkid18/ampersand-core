"""`ampersand-admin rotate-key` — mint a new API key + write it to the env file."""
...
from ampersand_core.server.admin_cli.config import AdminConfig, load_env_file
...
def run(cfg: AdminConfig, dry_run: bool = False) -> None:
    if cfg.env_file is None:
        typer.echo(
            ...
            "(default on a deployed box: /etc/ampersand/env).",
            err=True,
        )
    ...
    existing["AMPERSAND_API_KEY"] = new_key
    ...
    typer.echo("  sudo systemctl restart ampersand-server")
